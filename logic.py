...
class CricketGame:
    def __init__(self):
        ...
        self.balls = 0
        self.wickets = 0
        self.full = [0, 1, 2, 3, 4, 6, "NoBall", "Wicket", "Wide"]
        self.weights = [0.2, 0.15, 0.15, 0.15, 0.1, 0.1, 0.05, 0.05, 0.05]
        self.overs = 1
    ...
    def record(self, s):
        card_actions = {
            "NoBall": lambda: (self.score + 1, self.balls - 1),
            "Wide": lambda: (self.score + 1, self.balls - 1),
            "Wicket": lambda: self.wickets + 1
        }
        if isinstance(s, int):
            self.score += s
        else:
            card_actions.get(s, lambda: None)()
